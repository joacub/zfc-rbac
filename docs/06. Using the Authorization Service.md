...
use ZfjRbac\Service\AuthorizationServiceAwareTrait;
...
class Module
{
    ...
    public function getServiceConfig()
    {
        return [
            'initializers' => [
                'ZfjRbac\Initializer\AuthorizationServiceInitializer'
            ]
        ];
    }
}
...
ZfjRbac is shipped with a `ZfjRbac\Factory\AuthorizationServiceDelegatorFactory` [delegator factory]
...
is known to the ServiceManager as `'ZfjRbac\Service\AuthorizationService'`. Here is a classic example for injecting
        ...
        return [
            'factories' => [
                 'MyService' => function($sm) {
                     $authService = $sm->get('ZfjRbac\Service\AuthorizationService');
...
ZfjRbac comes with both a controller plugin and a view helper to check permissions.
