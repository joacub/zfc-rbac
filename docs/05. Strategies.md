...
happens when an access to a resource is unauthorized by ZfjRbac.

ZfjRbac strategies all check if an `ZfjRbac\Exception\UnauthorizedExceptionInterface` has been thrown.

By default, ZfjRbac does not register any strategy for you. The best place to register it is in your `onBootstrap`
...
public function onBootstrap(EventInterface $e)
{
    ...
    $t->getEventManager()->attach(
        $t->getServiceManager()->get('ZfjRbac\View\Strategy\UnauthorizedStrategy')
    );
}
```
...
ZfjRbac comes with two built-in strategies: `RedirectStrategy` and `UnauthorizedStrategy`.
...
> By default, ZfjRbac uses a template called `error/403`.
...
use ZfjRbac\View\Strategy\AbstractStrategy;
