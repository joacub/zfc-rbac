...
This section will help you to further understand how ZfjRbac works by providing more concrete examples. If you have
...
- [Using ZfjRbac with Doctrine ORM](/docs/07.%20Cookbook.md#using-zfcrbac-with-doctrine-orm)
    - [How to deal with roles with lot of permissions?](/docs/07.%20Cookbook.md#how-to-deal-with-roles-with-lot-of-permissions)
- [Using ZfjRbac and ZF2 Assetic](/docs/07.%20Cookbook.md#using-zfcrbac-and-zf2-assetic)
- [Using ZfjRbac and ZfcUser](/docs/07.%20Cookbook.md#using-zfcrbac-and-zfcuser)
...
Assuming the application example above we can easily use ZfjRbac to protect our route using the following guard:
        ...
        return [
            'factories' => [
                 'PostService' => function($sm) {
                     return new PostService(
                         $sm->get('doctrine.entitymanager.orm_default'),
                         $sm->get('ZfjRbac\Service\AuthorizationService') // This is new!
                     );
                 }
            ]
        ];
...
is not the task of such service. The Permission-System should handle this. And we can, for this we have the
 [`AssertionPluginManager`](/src/ZfjRbac/Assertion/AssertionPluginManager.php) and here is how to do it:
...
## Using ZfjRbac with Doctrine ORM

First your User entity class must implement `ZfjRbac\Identity\IdentityInterface` :
...
You need one more configuration step. Indeed, how can the RoleProvider retrieve your role and permissions ? For this you need to configure `ZfjRbac\Role\ObjectRepositoryRoleProvider` in your `zfc_rbac.global.php` file :
```php
        /**
         * Configuration for role provider
         */
        'role_provider' => [
            'ZfjRbac\Role\ObjectRepositoryRoleProvider' => [
...
Using DoctrineORM with ZfjRbac is very simple. You need to be aware from performances where there is a lot of permissions for roles.
...
## Using ZfjRbac and ZF2 Assetic

To use [Assetic](https://github.com/widmogrod/zf2-assetic-module) with ZfjRbac guards, you should modify your
...
return [
    'assetic_configuration' => [
        'acceptableErrors' => [
            \ZfjRbac\Guard\GuardInterface::GUARD_UNAUTHORIZED
        ]
    ]
];
```

## Using ZfjRbac and ZfcUser
