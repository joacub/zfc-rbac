...
Guards (called firewalls in older versions of ZfjRbac) are listeners that are registered on a specific event of
...
You can change it in ZfjRbac config, as follows:
...
ZfjRbac comes with four guards, in order of priority :
...
ZfjRbac is flexible enough to allow you to create custom guard. Let's say we want to create a guard that will
...
use ZfjRbac\Guard\AbstractGuard;
...
> Guards must implement `ZfjRbac\Guard\GuardInterface`.
...
Now, we just need to add the guard to the `guards` option, so that ZfjRbac execute the logic behind this guard. In
