...
`ZfjRbac\Role\RoleProviderInterface` interface. The only required method is `getRoles`, and must return an array
of `Rbac\Role\RoleInterface` objects.

Roles can come from any sources: in memory, from a file, from a database... However, please note that since ZfjRbac
...
Identity providers return the current identity. Most of the time, this means the logged user. ZfjRbac comes with a
default identity provider (`ZfjRbac\Identity\AuthenticationIdentityProvider`) that uses the
...
`ZfjRbac\Identity\IdentityProviderInterface` class. Then, change the `identity_provider` option in ZfjRbac config,
as shown below:
...
ZfjRbac comes with two built-in role providers: `InMemoryRoleProvider` and `ObjectRepositoryRoleProvider`. A role
...
To create a custom role providers, you first need to create a class that implements the `ZfjRbac\Role\RoleProviderInterface`
interface.
